...
parser.add_argument("--store", required=True, type=int, help='Store ID to be predict, only int')
...
def firefly(store: int = store_id.store) -> None:
    ...
    if not data:
        ...
        logger.info(f"Store Number {store} will sell between the worst scenario ${pred-average_MAE_training:.2f} and the best scenario ${pred+average_MAE_training:.2f} in the next 6 weeks.")
